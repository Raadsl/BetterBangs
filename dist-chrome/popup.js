document.addEventListener('DOMContentLoaded', () => {
  ...
  const firstResultEnabledCheckbox = document.getElementById('firstResultEnabled');
  ...
  fetch(browser.runtime.getURL('bangs.json'))
    ...
    .then(data => {
      ...
      browser.storage.local.get(
        { bangUsageCounts: {}, bangHistory: [], historyOptIn: false, blacklist: [], whitelist: [], useWhitelist: false, enableBlacklist: false, firstResultEnabled: false },
        (result) => {
          ...
          const firstResultEnabled = result.firstResultEnabled;
          ...
          firstResultEnabledCheckbox.checked = firstResultEnabled;
          ...
          optInHistoryCheckbox.addEventListener('change', (event) => {
            browser.storage.local.set({ historyOptIn: event.target.checked });
          });

          useWhitelistCheckbox.addEventListener('change', (event) => {
            browser.storage.local.set({ useWhitelist: event.target.checked });
          });

          enableBlacklistCheckbox.addEventListener('change', (event) => {
            browser.storage.local.set({ enableBlacklist: event.target.checked });
          });

          firstResultEnabledCheckbox.addEventListener('change', (event) => {
            browser.storage.local.set({ firstResultEnabled: event.target.checked });
          });
  ...
  function displayBangs(bangs) {
    ...
    browser.storage.local.get('historyOptIn', (result) => {
      const historyOptIn = result.historyOptIn;
  ...
  function displayHistory(history) {
    ...
    .forEach(date => {
      ...
      dateHeader.title = new Date(date).toLocaleDateString(undefined, {
          hour12: false
      });
      ...
      .forEach(item => {
        ...
        listItem.title = new Date(item.timestamp).toLocaleString(undefined, {
            hour12: false
        });

        const hourDate = new Date(item.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
  ...
  function formatDateLabel(date) {
    ...
    const historyDateString = historyDate.toLocaleDateString(undefined, {
        hour12: false
    });
    ...
    if (todayDateString === historyDateString) {
      ...
      return 'A week ago';
    } else if ((new Date(todayDateString) - new Date(historyDateString)) / (1000 * 60 * 60 * 24) <= 30) {
      return 'A month ago';
    } else if ((new Date(todayDateString) - new Date(historyDateString)) / (1000 * 60 * 60 * 24) <= 365) {
      return 'A year ago';
    } else {
  ...
  function handleSearch() {
    ...
    browser.storage.local.get('bangHistory', (result) => {
      const filteredHistory = result.bangHistory.filter(item => 
        ...
        new Date(item.timestamp).toLocaleString(undefined, { hour12: false }).toLowerCase().includes(query)
      );
  ...
  function deleteHistory() {
    if (confirm('Are you sure you want to delete the history?')) {
      browser.storage.local.set({ bangHistory: [] }, () => {
        displayHistory([]);
      });
    }
  }
  ...
  function addToList(listName, inputElement, listElement) {
    ...
    browser.storage.local.get({ [listName]: [] }, (result) => {
      const list = result[listName];
      if (!list.includes(bang)) {
        list.push(bang);
        browser.storage.local.set({ [listName]: list }, () => {
  ...
  function displayLastUpdated(lastUpdatedISO) {
    const lastUpdatedElement = document.getElementById('lastUpdated');
    lastUpdatedElement.title = "DuckDuckGo's bangs list is not updated frequently, so new bangs may take time to appear.";
    if (lastUpdatedISO && lastUpdatedElement) {
      try {
        ...
        lastUpdatedElement.textContent = `Bangs Last updated: ${formattedDate}`;
      } catch (error) {
        lastUpdatedElement.textContent = 'Bangs Last updated: Unknown';
      }
    }
  }
